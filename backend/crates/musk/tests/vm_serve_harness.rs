//! vm_serve_harness.rs — PLAN-044 T5: parity 换 VM 的服务 harness。
//!
...
impl VmServe {
    pub fn get(&self, path: &str) -> (u16, String) {
        ...
        let resp = std::panic::catch_unwind(|| ureq::get(&url).timeout(std::time::Duration::from_secs(15)).call());
        match resp {
            Ok(Ok(r)) => {
                let code = r.status();
            ...
            Ok(Err(e)) => panic!("GET {path} failed: {e}"),
            Err(_) => panic!("GET {path} panicked"),
        }
    }
}
