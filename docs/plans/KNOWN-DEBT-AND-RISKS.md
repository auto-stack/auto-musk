...
| 022 | **Phase 7.3 SecretaryMessage 留后**：§7.3 八组件清单中 SecretaryMessage 未移植。依赖 `useGateInbox` 完整状态机（registerGate 已由 forge_stream 的 gate_reached 回写部分覆盖，但 resolve/snooze/promoteNext/dismiss 状态机未移植）+ approve/reject API 接线（原生走 `/api/chats/session/{id}/approve/0` + `/reject-all`，relay gate 走 `/api/forge/relay/runs/{id}/gate`）。GateCard 的 approve/reject 当前只清 current_gate（UI 响应），真实 API 调用同批留后。 | `src/front/components/`（缺 SecretaryMessage.vue）/ `src/front/chats_view.at`（ApproveGate/RejectGate 仅清状态） |
