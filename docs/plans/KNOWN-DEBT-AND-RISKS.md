...
| 022 | **Phase 7.3 组件 props 绑定不支持 fn 调用**：.at 的组件 props 绑定（`Component { prop: expr }`）不支持 fn 调用表达式——`QuestionnaireCard { questions: getQuestions(.msg) }` 被 codegen 丢弃成 `:questions="null"`。降级方案：新增 `QuestionnaireMessage.vue` 包装组件（内部调 questionnaireFor，.at 只传 msg 字段访问）。其他需 fn 调用算 prop 的场景同理需包装组件规避。属 codegen 限制，登记备忘。 | `src/front/components/QuestionnaireMessage.vue`（规避产物）/ `auto-lang ui_gen` 组件 props 绑定 |
| 022 | **Phase 7.3 composable 声明自动调用**：widget use 块的 `composable: useX` 声明会生成 `const xLocal = useX()` 自动调用一次。若 composable 需要参数（如 useStreamingDocument 需 Ref<string>），自动调用会报错。降级：改用 `fn: useX` 声明（fn 不会自动调用，仅触发 ext 复制）。与 ext 复制限制同源（use 块声明兼"触发复制"与"引入使用"双职责）。 | `src/front/chats_view.at`（use 块 useStreamingDocument 声明） |
