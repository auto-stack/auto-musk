...
| 023 | **StreamingRenderer 保留永久逃生舱**（Plan 023 P3 20/21 原生化后唯一残留）：`useStreamingDocument(rawText: Ref<string>)` 需 Ref 参数（component fn 无法构造 computed 传入）+ 动态 `<component :is="registry[...]" v-bind>` 展开超 component fn 能力边界。023 交接文档 C1 已标记"流式/增量 JSON 难以纯原生"，登记 KNOWN-DEBT 永久保留。 | `src/front/components/StreamingRenderer.vue` |
