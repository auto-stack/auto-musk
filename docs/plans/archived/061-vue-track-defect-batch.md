...
- ⑨a D9 明文密码持久化:删除持久化(推荐,安全)还是保留回填便利?
  → **已裁定(2026-09-05)**:删 musk_login_password 明文,保留用户名回填
  (浏览器无真密文路径:WebCrypto 密钥同储=混淆;密码管理器不向页面
  代码开放回填)。
- ⑨b D10 品牌正名:AutoForge / Auto Musk / auto-musk 三选一,统一范围
  含 document.title?
  → **已裁定(2026-09-05)**:Auto Musk,三处统一(登录页标题/侧栏/
  document.title)。
- ⑨c D11 空状态与登录页 i18n 化是否入本批(涉及多模板,工作量中)?
  → **已裁定(2026-09-05)**:修,全部 i18n 化(D16 门禁兜底)。
- ⑨d D4 计划详情 Markdown/frontmatter 渲染是否入本批(需接线
  frontmatter util,工作量较大)?
  → **已裁定(2026-09-05)**:修,frontmatter 元数据条+Markdown 渲染。
  (四项均归下一计划执行——061 已终态归档。)
