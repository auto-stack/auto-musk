...
| **raw DropZone 闭环** | 调研纠正：**raw upload 端点后端早已存在**（wiki.rs + auto-src/wiki.at + auto_generated 三处，实测 multipart 上传可用）——缺口在前端契约层。①**api.at 补 4 端点**（wiki_raw_upload/mkdir/file/delete_file）→ api.ts 生成 5 个 raw 函数；②**逃生舱**：raw_upload.ts（XHR FormData + 进度 + rawFileUrl + loadRawFileText）+ DropZone 内联进 WikiNav.vue + RawPreview.vue（img/pdf/文本/下载，对齐原版 viewing-raw 分支）；③**wiki_store.at 补 handler**（UploadRawFiles/DeleteRawFile/CreateRawFolder/SelectRawFile/SetUploadProgress）；④**wiki_view.at 接线**（onSelectRaw/onDeleteRaw/onUploaded/onNewFolder + new-folder 表单 + raw 预览）。vue-tsc + vite build 全绿 | _待提交_ | ✅ raw 上传/删除/新建文件夹/预览闭环，构建通过 |
| **forge/mode 后端 + useForgeMode** | 原版 useForgeMode 是纯前端 localStorage，后端无 `/api/forge/mode`——新增后端能力。①**config 持久化**：MuskAppConfig 加 `forge_mode` 字段（hw app_config.rs + ag auto-src/app_config.at 双轨 + effective_forge_mode 默认 gsd）+ parity_app_config 同步；②**server.at 端点**：GET/PUT `/api/forge/mode`（ForgeModeBody 校验 gsd/check）+ build_router 注册 + 转译 auto_generated/server.rs；③**extern**：forge_mode_load/forge_mode_write（委托 hw app_config，写 config.at 仅覆盖 forge_mode 字段）；④**parity_forge_mode.rs 3 测试**（默认 gsd / PUT 持久化回读 / 非法拒绝）；⑤**前端**：api.at 声明 forge_mode_get/set → api.ts；SettingsMenu.vue 恢复 GSD/Check 分区（调后端替代 localStorage）；i18n 补 settings.mode 键。cargo test 全绿（含新增 3+6）；release 重建重启实测 GET/PUT 回读正确 | _待提交_ | ✅ /api/forge/mode 端点工作 + SettingsMenu GSD/Check 恢复 |
