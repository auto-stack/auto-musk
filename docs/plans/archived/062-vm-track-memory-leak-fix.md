...
updated_at: 2026-09-05T15:25:00+08:00

# Leave these EMPTY here — /auto-plan:review fills them:
supersedes_spec_components: []
new_spec_components:
  - "designs: PLAN-062 VM 轨内存生命周期契约——①timer 派发精确置脏(state_mutation_seq 写点计数×fire_timer 前后对比,空转拍不触发视图重建) ②帧域 retain 账本(retain_heap_result 记账→commit_dirty_frame 脏帧换代配平释放,AUTOUI_FRAME_RC=0 逃生门) ③桥接临时任务收尾清账(rc_release_task_stack [0,sp)) [Phase2 追加] ④stake 影子账本(VirtualRAM.stake_shadow:rc_push* 记/裸写清/清扫按影子;转移语义显式化 STORE_*/CONSTRUCT/POP/RET/桥接 takeover_heap_result;write_state 容器持有协议) ⑤宽限窗延迟回收(归零入 dying 队列 8192 解释步,复活出队,静止点 reap_all 确定性终态——回收契约从'归零即释'改为'归零入队+静止点释放') ⑥MCP 视图同步 dirty 门控(view_dirty/窗口尺寸/首同步 三条件,静止零重建)"
touched_goals: []
...
## Phase 2（2026-09-05 复活：残项根修）

> 用户裁定：KD-062①② 不再作为上游挂账，就地根修。本 phase 结束后重新归档。
> 已合回的 Phase 1（auto-musk 786cc08 / auto-lang merge d4819d2b2）与账本 P062-1..7 不变。

- **T10 20Hz 重建驱动定罪（auto-lang）** [✅ 已完成 2026-09-05] 无需新探针——现成 AUTO_DEBUG_KEYS=1 消息日志定罪：45s 计数 85×__hot_reload+85×PollStream+84×__bounds_collected（三个 500ms 泵 ~5.7msg/s）× 每消息过 view() 时 **MCP 同步块无条件全量模板重建**（renderer.rs view() 内 view_with_debug_gated+vtree 快照+read_all_state，mcp_shared 存在即跑、无客户端/绑定失败同形；855 rebuild-WARN/45s = 消息数×3.4 数学闭合）。
  dynamic_view 重建路径 + update 消息入口加 env 门控计数器（`AUTOUI_TRACE_FRAME`：
  每秒打印 重建次数 / view_dirty 置位来源分类 / 消息 event 频次 top-N）。
  实机（worktree auto.exe + 主检出源 + 9248 后端）跑 60s 读数定罪。
  验证：计数器输出唯一指认驱动源（候选：mouse_area on_move/hover 消息族、
  hot_reload_tick、MCP state-sync、订阅重建风暴）。

- **T11 20Hz 驱动修复（auto-lang）** [✅ 已完成 2026-09-05] MCP 同步块 dirty 门控（view_dirty[首帧恒真]/窗口尺寸比对[AppState.mcp_synced_ws 新增]/首次同步 三条件；静止视图零重建快照仍准确，异步脏写经 hot_reload 500ms 泵桥接至多 500ms 陈旧）。实机复验：重建 WARN 855/45s→20/50s（仅首帧），内存 265MB 静止十分钟级零增长（此前 1.25MB/s）——**空闲泄漏归零**。commit 06fb06239。
  依 T10 结论修复：消息驱动的置脏 → 只在 hover 目标实际变化时置脏（或缓存
  Element 自带 hover 态不重建）；订阅泵 → 无条件发射改条件发射。实机复验
  重建计数降到消息必需率（鼠标静止时 ~0）。
  验证：`AUTOUI_TRACE_FRAME` 60s 读数：静止态重建 ≤2/s → 目标 0/s；全量 nextest 差分零新增红。

- **T12 RC 槽位记账根修——stake 影子账本（auto-lang）** [✅ 已完成 2026-09-05] VirtualRAM.stake_shadow 平行账本（rc_push* 记/裸写清/清扫按影子）+ 转移语义显式化（STORE_*/CONSTRUCT_INSTANCE/POP/RET/桥接 takeover）+ write_state 家族容器持有协议 + 8192 步宽限窗延迟回收（帧内 raw 别名安全、静止点 reap_all 确定性终态）。soak 重建相收紧**零增长**转绿——KD-062① phantom stake 根除。宽域 187/190（3=基线红）。执行期定罪的次生缺陷：CONSTRUCT_INSTANCE 裸 push 丢份额（1000 对象滞留现场）、POP 内容释放对裸副本过度释放、write_state Rust 直写无持有（pkg props UAF）——全部随账本落地修复。
  VirtualRAM 增 `stake_shadow: Vec<u64>`（slot i ≠0 = 该槽拥有一份 id 的 stake）：
  rc_push/rc_push_id/rc_push_str_idx 写影子；裸写（push_nv/push_i32/push_str_idx/
  write_nv）清影子；pop 转移语义（STORE_GLOBAL）读走影子不释放；
  `rc_release_slot_range` 改按影子释放（替代按字节内容猜测——根治 raw 副本
  双重释放与陈旧字节误杀两类 canary 事故）；RET bp==0 帧清扫按影子。
  soak 重建相断言收紧：`≤24/拍` → 零增长（先红）。
  验证：soak 双相全零增长转绿；tests_rc_lifecycle 全绿；debug canary 零触发。

- **T13 全量门禁（auto-lang）** [✅ 已完成 2026-09-05] nextest 全量（最终提交）：见复审记录终态读数；vm-link-probe PASS。
  `cargo nextest run -p auto-lang --lib --features ui-iced --no-fail-fast` 差分
  零新增红（对照 Phase 1 基线 20 红族）；vm-link-probe PASS。
  验证：Summary 与基线红集一致。

- **T14 实机终验 + 再归档** [✅ 已完成 2026-09-05] 完整修复栈实机（T12 后二进制）：主实例 ~3.5min 存活窗口内 299→268MB 平坦（含初始化回落，19 采样点 ±0），rebuild-WARN 全程 40 条（仅首帧）——**空闲斜率 2.2MB/s→0 达成**（窗口被 KD-048a 静默退出收走，维持独立观察）。KD 062 行①② 已核销改写（残留观察：snapshot 子进程族+048a+nextest 必要性）。Phase 2 执行完毕，交 /auto-plan:review。
  worktree auto.exe 实机全家福会话 10min 采样：斜率 <5MB/min（目标 ~0）；
  KD-062①② 核销回写（062 行改写为已根治+指向本 phase commit）；计划
  status → reviewed → merge → archived 重走流程。
  验证：memdbg 采样曲线平直；KD/账本回写齐。
...
## Phase 2 复审（2026-09-05 15:00，用户复活计划后）

**方法**：worktree Phase2 双提交（06fb06239/cd5b11410）diff 对账（8 文件全在声明范围，无夹带）+ 全量门禁重跑 + 逐条验收复验 + 逃生门冒烟。

**逐条判定（Phase 2 视角）**：
1. 空闲 10min <50MB —— **pass**（T14：主实例 ~3.5min 存活窗口 299→268MB 平坦，折算 <10MB/10min；更长窗口被 KD-048a 静默退出阻断——独立观察在案）。
2. 钉子测试常驻 —— **pass**：soak 双相**零增长**（收紧后断言）/fire_timer_noop/mutation_seq（含 musk 形态）/plan051 族全绿。
3. 发送-轮询-完成活体 —— **partial（与 Phase 1 同判，不在 Phase 2 域）**：KD-047 抛点+env 无回复双阻塞，用户已认。
4. 全门禁差分 —— **pass**：nextest 4530 绿/19 红，五族（layout/charts/d8/c2/strips）**逐一经 master 旁挂 worktree 对照基线既有**——strips 为 Phase 2 复审期新增对照项（master 同红，并行会话引入）。零新增红。
5. canary+逃生门 —— **pass**：canary 契约修订后全绿（仪器在 T12 开发期捕获 3 次真实过度释放——保护价值实证）；AUTOUI_FRAME_RC=0 冒烟：soak 重建相 +840 红回泄漏态=逃生门有效。
6. KD 回写 —— **pass**：062 行①②核销改写（含 snapshot 子进程族/048a/nextest 必要性三项残留观察）。

**遗漏/延后/workaround**：
- 遗漏：无（8 文件 diff 全对账；soak 断言收紧链在提交注透明）。
- 延后（在案）：048a 静默退出（独立观察，本批修复版同形复现 ~4-5min）；snapshot 子进程族（新观察：66MB 单例+多 43MB 瞬态）；发送完成半段（KD-047）。
- Workaround 判定：**宽限窗=设计性回收策略**（有界 8192 步/静止点确定性终态/测试化），非 hack；write_state 防御性补持=保守方向显式记账。**诚实记录**：plan-419 两测试的契约语义被修订（"归零即释"→"归零入队+静止点释放"）——canary 对 post-reap UAF 的保护保留。
- 执行事故记录：复现实验中曾在临时 worktree 误建目录级锚（确认为空目录非符号链接，无红线违反，已随 worktree 清理）。

**门禁读数**：nextest 4549 run/4530 绿/19 红（与 T13 一致）；焦点电池 52/1（c2 基线）。

**路由**：`status: reviewed`（Phase 2 全根修验证通过）→ 交 /auto-plan:merge。
