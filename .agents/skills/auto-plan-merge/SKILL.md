...
description: |
  ...
  docs/plans/archived/, setting status to merged. Use when:
  (1) User says "merge plan N" / "沉淀 plan N" / "归档 plan N" / "把 plan 并入 spec"
  (2) User says "/auto-plan:merge" or a plan is review_done and ready to settle
  (3) A plan has passed /auto-plan:review and the user wants its outcome to become permanent spec knowledge
  This skill refuses any plan that is not review_done. It prefers the
...
Take a `review_done` plan, extract its knowledge into the spec ledger's 6
sections (each new spec item traces back to the plan via `file` + `related`),
move the plan into `archived/`, and mark it `merged`. One skill, one session, one
plan.
...
**State gate (HARD):** The plan **must** be `review_done`. Refuse anything else
...
```bash
# Verify status == review_done before doing anything:
head -10 docs/plans/<NNN>-*.md | grep 'status:'
```

If not `review_done`, **stop**. Do not "merge anyway" — tell the user what status
...
`docs/plans/archived/` with `status: merged`, and `specs.json` grew. If so, skip
to Step 5.
...
Then set `status: merged` in the moved file's frontmatter (this is the terminal
state — `merged` plans do not go back).

### Step 5: Verify + report

- The plan is under `docs/plans/archived/` with `status: merged`.
...
- **Never merge a plan that is not `review_done`.** The review gate exists for a reason.
...
- **Always archive + set `merged`.** A merged plan leaves `docs/plans/` for `archived/`.
...
- [ ] Plan confirmed `review_done` (gate passed)
...
- [ ] `status: merged` set in the archived plan's frontmatter
