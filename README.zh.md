...
| Spec Ledger | 结构化目标、架构、设计、测试、复审与报告六区 | 工作区 `.autoos/specs.json` |
| 模块规范 | 当前模块行为与设计知识 | `docs/specs/` |
| 会话 | 对话与 Relay 活动历史 | 工作区 `.autoos/conversations/` |

当前沉淀分为两部分：`merge_plan` 将 Plan 章节映射到 ledger 条目并归档计划；document 阶段的 Agent 再根据复审填写的影响元数据更新模块规范。这两部分目前是分别执行的操作。
...
Plan 正文包括目标、架构、背景分析、详细设计、测试设计、验收标准、执行任务、复审记录与待澄清事项。Frontmatter 保存 ID、状态、进度，以及复审填写的 `supersedes_spec_components`、`new_spec_components`、`touched_goals`。
...
1. **统一流程契约。** 对齐技能、运行时提示、状态迁移、worktree 命名及 Auto 与生成实现，将确定性校验下沉为共享工具。
2. **允许有边界的计划调整。** 以 Plan 为主上下文，允许按需查阅相关代码与规范；区分局部实现调整和需要重新确认的需求变更。
...
5. **让沉淀过程可恢复。** 明确规范的新增、修改与废弃，分别记录代码合入、ledger 与模块规范更新、归档和清理；明确模块规范与 ledger 视图之间的权威关系。
