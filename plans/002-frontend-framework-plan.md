...
- **DSL 风格定调**：Step 3 出于稳妥先用 015-notes 基础元素（`row/col/link/text/button`）搭起骨架并验证生成链路。**后续可升级使用 component-gallery 已验证的全套组件**（nav-link/icon/theme-toggle/card/table/dialog 等，见 §2.1），让 NavRail 和各视图接近 auto-forge 视觉。`link (to: "/x")` → `<router-link>`（含 active-class），生成质量好。
