...
### Step 3 — App Shell + 导航 + 路由
- `app.at` 写 routes（登录/chats/explorer/specs/relay/...）+ 左侧 nav rail + outlet
- 参照 component-gallery app.at 的布局/导航写法（DSL 层面）
- 各视图先占位

### Step 4 — 登录视图 + explorer 样板视图
- `login.at`：表单 + 提交逻辑（前端校验）
- `explorer.at`：列表（mock 数据）
