<!--
  MentionDropdown.vue — @mention 下拉（逃生舱，从 web/ 移植）
  Plan 022 Phase 7c: defineExpose 键盘导航（moveUp/moveDown/currentId/hasItems），
  供 MentionInput.vue 的 handleKeydown 调用。AgentAvatar 用本目录精简版。
-->
<template>
  <Teleport to="body">
...
const filtered = computed(() => {
  ...
  return props.professions.filter(
    (p) =>
...
defineExpose({
  ...
  moveDown() {
    index.value = Math.min(filtered.value.length - 1, index.value + 1)
  },
  currentId(): string | undefined {
    return filtered.value[index.value]?.id
  },
  hasItems(): boolean {
...
.mention-item:hover,
.mention-item.active {
  background: hsl(var(--primary, 220 90% 56%) / 0.08);
}
.mention-item.active {
  color: var(--af-primary, hsl(220 90% 56%));
}
.mention-name {
  font-weight: 600;
...
.mention-label {
  color: var(--af-muted, hsl(220 9% 46%));
  font-size: 0.83rem;
}
</style>
