<!--
  ReportCard.vue — Relay 完成报告卡片（逃生舱，从 web/ 移植）
  Plan 022 Phase 7.3: 显示 run_completed 事件的报告（runId/goalsMet/testsPass/
  driftDetected/cost/confidence/deliverables）。props: { report }, emit view-full/
  download/open-files。lucide-vue-next 已在 gen 依赖。
-->
<template>
  <div class="report-card" :class="{ collapsed: !expanded }">
    <div class="report-header" @click="expanded = !expanded">
      ...
      <span class="report-confidence" :class="(report.confidence || 'medium').toLowerCase()">
    ...
    <div v-if="expanded" class="report-body">
      ...
      <div v-if="report.deliverables && report.deliverables.length > 0" class="report-deliverables">
      ...
      <div class="report-actions">
        ...
        <button class="report-btn" @click.stop="$emit('open-files')">
          <FolderOpen :size="13" /> Open Changed Files
        </button>
      </div>
    </div>
  </div>
...
.report-header {
  ...
  padding: 0.6rem 0.8rem; cursor: pointer; user-select: none;
}
.report-header:hover { background: hsl(142 70% 45% / 0.06); }
.report-status { font-size: 1rem; flex-shrink: 0; }
.report-title { flex: 1; font-size: 0.93rem; font-weight: 500; color: var(--af-fg, hsl(220 14% 10%)); }
.report-confidence {
  font-size: 0.73rem; padding: 0.15rem 0.4rem; border-radius: 4px;
...
.report-chevron { color: var(--af-muted, hsl(220 9% 46%)); flex-shrink: 0; }
.report-body {
...
.metric-row {
  ...
  padding: 0.3rem 0.4rem; background: hsl(220 14% 50% / 0.04);
  border-radius: 5px; font-size: 0.83rem;
}
.metric-label { color: var(--af-muted, hsl(220 9% 46%)); }
.metric-value { font-weight: 500; color: var(--af-fg, hsl(220 14% 10%)); }
.metric-value.drift { color: hsl(0 72% 51%); }
.section-title {
  font-size: 0.78rem; font-weight: 600; text-transform: uppercase;
  color: var(--af-muted, hsl(220 9% 46%)); letter-spacing: 0.03em; margin-bottom: 0.2rem;
}
.report-deliverables ul { margin: 0; padding-left: 1.1rem; font-size: 0.88rem; color: var(--af-fg, hsl(220 14% 10%)); line-height: 1.5; }
.report-deliverables li { margin: 0.1rem 0; }
.report-actions { display: flex; flex-wrap: wrap; gap: 0.35rem; }
.report-btn {
  display: inline-flex; align-items: center; gap: 0.3rem;
  padding: 0.35rem 0.6rem; border: 1px solid var(--af-border, hsl(220 13% 91%));
  border-radius: 5px; background: transparent; color: var(--af-fg, hsl(220 14% 10%));
  font-size: 0.83rem; font-weight: 500; cursor: pointer; transition: all 0.15s;
}
.report-btn:hover { background: hsl(220 14% 50% / 0.06); border-color: hsl(220 90% 56% / 0.3); }
</style>
