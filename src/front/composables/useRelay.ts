...
export interface RunEventDto {
  ...
  step_id?: string
  profession_id?: string
  // B 类批3：后端 RunEvent 用 role_id（relay_store.at），前端兼容两字段名
  role_id?: string
...
export function useRelay() {
  ...
  async function loadRuns(projectPath?: string) {
    try {
      ...
      const resp = await fetch(`${API_BASE}/runs${query}`)
      if (!resp.ok) throw new Error(`Failed: ${resp.status}`)
  ...
  function eventsToSessionLog(runId: string, events: RunEventDto[]): SessionLogEntry[] {
    ...
    for (const ev of events) {
      ...
      const prof = ev.role_id || ev.profession_id || 'unknown'
      switch (ev.type) {
        case 'turn_delta':
  ...
  async function updateRunTitle(runId: string, title: string) {
    try {
      const resp = await fetch(`${API_BASE}/runs/${runId}/title`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  function subscribeToRun(runId: string, onEvent?: (event: any) => void) {
    ...
    // B 类批3：EventSource 不能设 header，沿用 forge_stream 的 query token 鉴权模式
    // （relay events 与 chat_stream 同 recipe，支持 ?token=&workspace=）。
    const ws = localStorage.getItem('musk_workspace')
    const tk = localStorage.getItem('musk_jwt')
    const params: string[] = []
    if (ws) params.push(`workspace=${encodeURIComponent(ws)}`)
    if (tk) params.push(`token=${encodeURIComponent(tk)}`)
    const qs = params.length ? '?' + params.join('&') : ''
    const es = new EventSource(`${API_BASE}/runs/${runId}/events${qs}`)
    es.onmessage = (event) => {
      try {
        ...
        // B 类批3：后端 RunEvent 用 role_id（relay_store.at:44-58），原生前端误读 profession_id
        // 导致会话日志 profession 全 unknown。此处对齐后端字段。
        const prof = data.payload?.role_id || data.payload?.profession_id || 'unknown'
