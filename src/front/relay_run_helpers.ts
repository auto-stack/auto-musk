...
/** 在当前 run 或 runs 列表里找 run（对齐原 run computed：currentRun 优先 + find 兜底）。 */
export function relayFindRun(runs: any[], currentRun: any, runId: string): any {
  if (currentRun?.run_id === runId) return currentRun
  return runs.find((r) => r.run_id === runId) ?? currentRun
}
...
/** 职业→图标（.at 无字典字面量）。 */
export function relayProfessionIcon(id: string): string {
  const map: Record<string, string> = {
...
/** run?.title || runId（.at 无 ?. 与 || 语义的简写）。 */
export function relayRunTitle(run: any, runId: string): string {
  return run?.title || runId
}

/** run?.waiting_for_gate（None 安全）。 */
export function relayWaitingGate(run: any): any {
  return run?.waiting_for_gate ?? null
}

/** entry-<type> class（7 分支 type 链用 fn 收敛，避免 .at 超长嵌套 if）。 */
export function relayEntryClass(type: string): string {
  return `log-entry entry-${type}`
}
